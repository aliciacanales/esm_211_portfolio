...
# makes bad ice years more likely if the previous year was bad (use 80% likelihood).
# Bad -> good
...
plot(Pbear_task1_projection) ## population size starts to decrease by the 5th year and plateau after year 10


# Task 2:
# Defining 2 new parameters which describe the probability next year is a bad year given that this year was a good  (use 50% likelihood), and (b) describe the
# probability next year is a bad year given that this year was a bad year (use 80% likelihood again).
# Good -> bad
...
Pbear_task2_matrix <- matrix(c(rep(c((1-p5)/3, (1-p5)/3, (1-p5)/3, p5/2, p5/2), 3),
                      rep(c((1-q5)/3, (1-q5)/3, (1-q5)/3, q5/2, q5/2), 2)), 5, 5) ## making another matrix

Pbear_task2_projection <- project(Pbear, Pbear_vector, Aseq = Pbear_task2_matrix, time = 50)
plot(Pbear_task2_projection) ## about the same but there is now a little lip at the first 1-3 years.
